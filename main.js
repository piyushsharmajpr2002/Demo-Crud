async function callAPi(){
    let result = await fetch('https://personal-vtuksnns.outsystemscloud.com/BasicConcepts_CS/rest/Studends/RestApiGet')
    result = await result.json() 
    console.warn(result)
    document.getElementById('userData').innerHTML=result
    .map((user)=> `<tr>
    <td>${user.RollNo}</td>
    <td class="find">${user.FirstName}</td> 
    <td>${user.LastName}</td>
    <td>${user.Address}</td>
    ...
    <td><button type="button" class="btn btn-outline-secondary" onclick="Show_Student_Editbox(${user.RollNo})">Edit</button>
    <button type="button" class="btn btn-outline-danger" onclick="userDelete(${user.RollNo})">Del</button></td>
    </tr>` 
    ).join(""); } 
     callAPi()
